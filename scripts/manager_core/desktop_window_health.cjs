...
(() => {
  ...
  app.on('browser-window-created',(_,win)=>{
    ...
    const state={win,ready:false,pending:0,started:0,finished:0,latency:0,failures:0};
  ...
  const timer=setInterval(async()=>{
    ...
    try{
      ...
      if(state.ready&&!state.pending&&now-state.started>=2000){
        ...
        Promise.resolve().then(()=>state.win.webContents.executeJavaScript('0',false)).then(()=>{
      ...
      const data={version:1,appPid:process.pid,hwnd:lease.hwnd,token:lease.token,at:now,
        ...
        renderer_roundtrip_ms:state.latency,renderer_completed_at:state.finished,failures:state.failures};
