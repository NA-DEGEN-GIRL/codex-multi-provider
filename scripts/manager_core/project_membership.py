...
def project_memberships(state, home, canonical=None, *, signals=None):
    ...
    signals = Path(signals or database.parent / '.manager-record-signals')
    proven = membership_proofs.read(signals)
    observed = set()
    with closing(sqlite3.connect(database.resolve().as_uri() + '?mode=ro', uri=True, timeout=5)) as db:
        ...
        for thread, native in db.execute('SELECT id,project_id FROM threads'):
            ...
            elif native is None and previous is not None and (
                    thread in proven or migration.get('threadAssignmentsReadMigrated') is True):
                assignments.pop(thread)
                projectless.add(thread)
    membership_proofs.remember(signals, observed)
